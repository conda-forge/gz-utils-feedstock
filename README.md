...
Once the `conda-forge` channel has been enabled, `gz-utils, gz-utils3, libgz-utils, libgz-utils3` can be installed with `conda`:

```
conda install gz-utils gz-utils3 libgz-utils libgz-utils3
```

or with `mamba`:

```
mamba install gz-utils gz-utils3 libgz-utils libgz-utils3
```

It is possible to list all of the versions of `gz-utils` available on your platform with `conda`:
